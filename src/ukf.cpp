...
void UKF::ProcessMeasurement(MeasurementPackage meas_pack) {
 ...
 double dt = ( meas_pack.timestamp_ - previous_timestamp_ )/1000000.0;
  ...
  if( dt > 1./pow(10, 5) )
  {
    Prediction( dt ); //single prediction function will handle the state predictions
  }
...
void UKF::Prediction(double delta_t) {
 ...
 for( int pt = 0; pt < 2*n_aug_+1; pt++ )
 {
     ...
     double si     = Xsig_aug_(3,pt);
     double si_d    = Xsig_aug_(4,pt);
     double nu_a     = Xsig_aug_(5,pt);
     double nu_si_dd = Xsig_aug_(6,pt);

     if( si_d == 0. )
     {
       Xsig_pred_(0,pt) = x + v*cos(si)*dt + 0.5f*dt2*cos(si)*nu_a;
       Xsig_pred_(1,pt) = y + v*sin(si)*dt + 0.5f*dt2*sin(si)*nu_a;
     }
     else
     {
       Xsig_pred_(0,pt) = x + ( v/si_d )*( sin(si + si_d*dt) - sin(si) )
                         + 0.5f*dt2*cos(si)*nu_a;
       Xsig_pred_(1,pt) = y + ( v/si_d )*( -cos(si + si_d*dt) + cos(si) )
                         + 0.5f*dt2*sin(si)*nu_a;
     }
     Xsig_pred_(2,pt) = v    +           dt*nu_a;
     Xsig_pred_(3,pt) = si  + si_d*dt + 0.5f*dt2*nu_si_dd;
     Xsig_pred_(4,pt) = si_d +           dt*nu_si_dd;
 }
...
void UKF::UpdateLidar(MeasurementPackage meas_pack) {
  ...
  */
  ...
  //Trnsformation of sigma points to measurement space
  for( int pt = 0; pt < 2*n_aug_ + 1; pt++ )
 {
 ...
 //calculate the NIS for the laser and write it to the NIS laser txt file
...
void UKF::UpdateRadar(MeasurementPackage meas_pack) {
  ...
  */
  ...
  for( int pt = 0; pt < 2*n_aug_ + 1; pt++ )
  {
    ...
    double si = Xsig_pred_(3,pt);
    Zsig_radar_(0,pt) = sqrt(px*px+py*py);
    Zsig_radar_(1,pt) = atan2(py,px);
    Zsig_radar_(2,pt) = ( px*cos(si)*v + py*sin(si)*v )/sqrt(px*px+py*py);
  }

  //perform update operations in the measurement space
  ...
  //calculate Kalman gain K_radar_;
 ...
 //calculate the NIS for the radar and write it to the NIS radar txt file
